...
-- Ensuring that this script uses the Chinook databse when it runs.
USE Chinook;
...
SELECT COUNT(*), SUM(Total) FROM Invoice WHERE YEAR(InvoiceDate) = 2021;

-- (challenge: find the invoice count sales total for every year using one query)
SELECT COUNT(*), SUM(Total) FROM Invoice GROUP BY YEAR(InvoiceDate);
...
SELECT SUM(Total) FROM Invoice GROUP BY BillingCountry ORDER BY SUM(Total);
